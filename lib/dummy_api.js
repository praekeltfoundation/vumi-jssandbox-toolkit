...
function DummyApi() {
    ...
    self._http_request_key = function(url, content_type, body) {
        if (body && utils.starts_with(content_type, "application/json")) {
            body = self._order_json_data(body);
        }

        return [url, content_type, body].join('|');
    };

    self._http_request_key_for_fixture = function(fixture) {
        var req = fixture.request;
        return self._http_request_key(req.url, req.content_type, req.body);
    };

    self._http_request_key_for_cmd = function(cmd) {
        var headers = cmd.headers || {};
        var content_types = headers['Content-Type'] || [];
        return self._http_request_key(cmd.url, content_types[0], cmd.data);
    };

    self.add_http_fixture = function(fixture) {
        utils.set_defaults(fixture, {response: {}});
        ...
        var key = self._http_request_key_for_fixture(fixture);
        self.http_fixtures[fixture.request.method][key] = fixture.response;
    };
    ...
    self._handle_metrics_fire = function(cmd) {
        var success = self._record_metric(
            cmd.store, cmd.metric, cmd.value, cmd.agg);
        return self._populate_reply(cmd, {success: success});
    };

    // http resource

    self._record_http_request = function(method, cmd) {
        self.http_requests.push({
            method: method,
            url: cmd.url,
            body: cmd.data,
            headers: cmd.headers
    ...
    self._reply_to_http_request = function(method, cmd) {
        var key = self._http_request_key_for_cmd(cmd);
        var response = self.http_fixtures[method][key];

        if (typeof response == 'undefined') {
            return self._reply_fail(cmd, method + " from unknown key: " + key);
        }
    ...
    self._handle_http_method = function(method, cmd) {
        self._record_http_request(method, cmd);
        return self._reply_to_http_request(method, cmd);
    };

    self._handle_http_get = function(cmd) {
        return self._handle_http_method("GET", cmd);
    };

    self._handle_http_head = function(cmd) {
        return self._handle_http_method("HEAD", cmd);
    };

    self._handle_http_post = function(cmd) {
        return self._handle_http_method("POST", cmd);
    };

    self._handle_http_put = function(cmd) {
        return self._handle_http_method("PUT", cmd);
    };

    self._handle_http_delete = function(cmd) {
        return self._handle_http_method("DELETE", cmd);
    };
