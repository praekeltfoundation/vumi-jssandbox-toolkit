...
function DummyApi() {
    ...
    self._http_post_key = function(url, data) {
        try {
    ...
    self.add_http_fixture = function(fixture) {
        if (fixture.method == "POST") {
            var key = self._http_post_key(fixture.url,
    ...
    self._record_metric = function (store_name, metric_name, value, agg) {
        ...
        if (typeof metric == 'undefined') {
            store[metric_name] = metric = {'agg': agg};
        }
    ...
    self._handle_http_post = function(cmd, reply) {
        var key = self._http_post_key(cmd.url, cmd.data);
