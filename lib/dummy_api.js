...
function DummyApi() {
    ...
    self._handle_contacts_search = function(cmd, reply) {
        // only allows exact matches for now (i.e. no fancy Lucerne queries)
        var results = [];
        var terms = Object.keys(cmd).filter(function (key) { return key != 'cmd' })
        for (var contact in self.contact_store) {
            var match = terms.every(function (term, i, a)  {
                return (JSON.stringify(self.contact_store[contact][term])
                        == JSON.stringify(cmd[term]))
            })
            if (match) {
                results.push(self.contact_store[contact])
            }
        }
        self._reply_success(cmd, reply, {success: true, contacts: results});
    };
