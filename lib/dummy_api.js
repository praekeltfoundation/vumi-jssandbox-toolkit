...
function DummyApi() {
    ...
    self.add_http_fixture = function(fixture) {
        ...
        var body_data = JSON.stringify(fixture.body);

        if (typeof fixture.data != "undefined") {
            var content_type = fixture.content_type || "application/json";
            var data = self._format_http_request_data(
                content_type, fixture.data);
                var key = self._http_request_key(
                    fixture.method, fixture.url, content_type, data);
                    self.http_calls[fixture.method][key] = body_data;
        } else {
            self.http_calls[fixture.method][fixture.url] = body_data;
        }
    };
    ...
    self._handle_metrics_fire = function(cmd) {
        var success = self._record_metric(
            ...
            return self._reply_populate_reply(cmd, {success: success});
    };

    // http resource

    self._handle_http_method = function(method, key, cmd) {
        ...
        return self._reply_success(cmd, reply_data);
    };

    self._handle_http_get = function(cmd) {
        return self._handle_http_method("GET", cmd.url, cmd);
    };
    ...
    self._handle_http_post = function(cmd) {
        ...
        var key = self._http_request_key(
            'POST', cmd.url, content_type, cmd.data);
            return self._handle_http_method("POST", key, cmd);
    };
    ...
    self._handle_http_put = function(cmd) {
        ...
        var content_type = content_types[0];
        var key = self._http_request_key(
            'PUT', cmd.url, content_type, cmd.data);
            return self._handle_http_method("PUT", key, cmd);
    };
    ...
    self._handle_http_delete = function(cmd) {
        ...
        return self._handle_http_method("DELETE", key, cmd);
    };
    ...
    self._handle_http_head = function(cmd) {
        ...
        return self._handle_http_method("HEAD", key, cmd);
    };
