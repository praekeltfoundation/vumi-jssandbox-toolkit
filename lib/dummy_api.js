...
function DummyApi() {
    ...
    self.set_group_search_results = function(query, group_keys) {
      this.group_search_store[query.toLowerCase()] = group_keys;
    };
    ...
    self._handle_groups_get_by_name = function(cmd, reply) {
      ...
      if(matches.length === 0) {
        ...
        self._reply_fail(cmd, reply, 'More than one group returned');
      } else {
    ...
    self._handle_groups_count_members = function(cmd, reply) {
      ...
      if(!group) {
        ...
        if(is_smart_group) {
          ...
          var contacts = Object.keys(self.contact_store).filter(function(contact_key) {
