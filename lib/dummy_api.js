...
function DummyApiError(msg) {
    this.name = "DummyApiError";
    this.message = msg;
}
DummyApiError.prototype = new Error();
DummyApiError.prototype.constructor = DummyApiError;
...
function DummyApi() {
    ...
    self.noop = function (original) { return original; };
    self.fixture_encoders = {
        'application/json': JSON.stringify,
        'text/plain': self.noop
    };
    self.fixture_default_encoder = self.fixture_encoders['application/json'];

    self.add_fixture_encoder = function(content_type, encoder) {
        self.fixture_encoders[content_type] = encoder;
    };
    ...
    self.add_http_fixture = function(fixture) {
        var method_key = fixture.method.toUpperCase();
        var url_key;
        ...
        var encoder = self.fixture_encoders[fixture.content_type] || self.fixture_default_encoder;
        self.http_calls[method_key][url_key] = encoder(fixture.body);
    };
    ...
    self.set_contact_search_results = function(query, contact_keys) {
        this.contact_search_store[query] = contact_keys;
    };

    self.add_group = function(group) {
        var set_default = function(name, value) {
            group[name] = ((typeof group[name] !== 'undefined') ?
                             group[name] : value);
        };

        set_default('key', self._generate_key());
        set_default('name', null);
        set_default('query', null);

        this.group_store[group.key] = group;
        return group;
    };

    self.set_group_search_results = function(query, group_keys) {
      this.group_search_store[query] = group_keys;
    };
    ...
    self.set_smart_group_query_results = function(query, contact_keys) {
      this.smart_group_query_store[query] = contact_keys;
    };
    ...
    self._handle_contacts_get_by_key = function(cmd, reply) {
        var key = cmd.key || null;
        if (!self.contact_store[key]) {
            self._reply_fail(cmd, reply, "Contact not found");
            return;
        }
        self._reply_success(cmd, reply, {
            contact: self.contact_store[key]
        });
    };

    self._handle_contacts_search = function(cmd, reply) {
        var query = cmd.query || null
        var matches = self.contact_search_store[query] || [];
        self._reply_success(cmd, reply, {
            keys: matches
        });
    };
    ...
    // groups resource
    self._handle_groups_list = function(cmd, reply) {
      self._reply_success(cmd, reply, {
        groups: Object.keys(self.group_store).map(function(key) {
          return self.group_store[key];
        })
      });
    };

    self._handle_groups_search = function(cmd, reply) {
      var matched_keys = self.group_search_store[cmd.query.toLowerCase()] || [];
      self._reply_success(cmd, reply, {
    ...
    self._handle_groups_get = function(cmd, reply) {
    ...
    self._handle_groups_get_by_name = function(cmd, reply) {
      var group_keys = Object.keys(self.group_store);
      ...
      if(matches.length === 0) {
        ...
        self._reply_fail(cmd, reply, 'Multiple groups found');
      } else {
        var group = self.group_store[matches[0]];
        self._reply_success(cmd, reply, {group: group});
      }
    };

    self._handle_groups_get_or_create_by_name = function(cmd, reply) {
      var group_keys = Object.keys(self.group_store);
      var matches = group_keys.filter(function(group_key) {
        var group = self.group_store[group_key];
        return group.name == cmd.name;
      ...
      if(matches.length === 0) {
        self._reply_success(cmd, reply, {
          created: true,
          group: self.add_group({key: this._generate_key(), name: cmd.name})
        });
      } else if (matches.length > 1) {
        self._reply_fail(cmd, reply, 'Multiple groups found');
      } else {
    ...
    self._handle_groups_update = function(cmd, reply) {
      var group = self.group_store[cmd.key];
      if(!group) {
        self._reply_fail(cmd, reply, 'Group not found');
      } else {
    ...
    self._handle_groups_count_members = function(cmd, reply) {
      var group = self.group_store[cmd.key];
      if(!group) {
        self._reply_fail(cmd, reply, 'Group not found');
      } else {
        ...
        if(is_smart_group) {
          ...
          contacts = Object.keys(self.contact_store).filter(function(contact_key) {
