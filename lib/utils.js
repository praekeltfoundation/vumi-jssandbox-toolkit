...
function pretty(obj, first, last) {
    if (_.isArray(obj)) {
        return yaml.safeDump(obj, {indent: 4});
    }

    var all = _.keys(obj);
    first = _.intersection(first || [], all);
    last = _.intersection(last || [], all);
    var rest = _.difference(all, first, last);

    return []
        .concat(first, rest, last)
        .map(function(k) {
            var item = _.zipObject([k], [obj[k]]);
            return yaml.safeDump(item, {indent: 4});
        })
        .join('');
}
