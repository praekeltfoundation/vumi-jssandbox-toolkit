...
var User = Eventable.extend(function(im) {
    /**:User(im)
    ...
    */
    ...
    self.setup = function(addr, opts) {
        ...
        **/
        ...
        self.state = null;
        if (opts.state) {
            self.set_state(opts.state.name, opts.state.metadata);
        }
    ...
    self.set_state = function(name, metadata) {
        /**:User.set_state(name[, metadata])
        Sets the user's current state (the state most recently visited by the
        user). The name of the state, as well as optional metadata are
        recorded. The metadata can always be updated at a later stage with
        :meth:`User.update_state_metadata`. The state metadata is only
        relevant to the current state the user is on, so subsequent calls
        to :meth:`User.set_state` will reset the user's current state metadata.

        :param string name:
            the name of the new current state
        :param object metadata:
            optional metadata about the new state.
        */
        self.state = {};
        self.state.name = name;
    ...
    self.update_state_metadata = function(metadata) {
        /**:User.set_state(name[, metadata])
        Update the metadata about the current state the user is on. Any
        properties in the current metadata with the same names as properties in
        the new metadata will overwritten.

        :param object metadata:
            the new metadata to add.
        */
        var state = self.state;
        utils.update(state.metadata, metadata || {});
        return self;
    };
