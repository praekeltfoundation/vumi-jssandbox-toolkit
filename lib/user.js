...
function User(im) {
    ...
    */
    ...
    self.load = function(addr, opts) {
        ...
        */
        ...
        return self.fetch().then(function(data) {
            if (!data) {
                throw new Error("Failed to load user '" + self.addr + "'");
            }
    ...
    self.key = function() {
        ...
        */
        return self.store_name
            ? "users." + self.store_name + '.' + self.addr
            : "users." + self.addr;
    };
    ...
    self.set_current_state_name = function(state) {
        /**:User.set_current_state_name(state)
        Gives the user a new language. If the user's language has changed,
        their jed gettext object is refreshed (delegates to
        :meth:`User.refresh_i18n`). Returns a promise that will be
        fulfilled once the method's work is complete.
        ...
        */
        self.current_state_state = state.name;
        return self;
    };
    ...
    self.fetch = function() {
        ...
        */
        return self.im
            .api_request("kv.get", {key: self.key()})
            .then(function (reply) {
                return reply.value;
            });
    };
