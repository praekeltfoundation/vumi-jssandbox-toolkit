...
var HttpApi = Eventable.extend(function(self, im, opts) {
    ...
    */
    ...
    self.set_defaults = function(opts) {
        opts = _.defaults(opts || {}, {headers: {}});

        var headers = _.extend({}, self.defaults.headers);

        if (self.defaults.auth) {
            headers.Authorization = [self.make_auth(
                self.defaults.auth.username,
                self.defaults.auth.password)];
        }

        return _.defaults(opts, _.omit(self.defaults, 'headers', 'auth'), {
            encoder: self.encode_request_data,
            headers: _.defaults(opts.headers, headers)
        });
    };
    ...
    self.request = function(method, url, opts) {
        ...
        */
        var request = new HttpRequest(method, url, self.set_defaults(opts));
        request.encode();
