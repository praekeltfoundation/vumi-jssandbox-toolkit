...
var ChoiceState = State.extend(function(self, name, opts) {
    ...
    :type opts.choices: Array of :class:`Choice` objects
    :param opts.choices:
        ordered list of choices to display
...
var LanguageChoice = ChoiceState.extend(function(self, name, opts) {
    ...
    */
    ...
    self.save_response = function(response) {
        ...
        self.im.user.set_lang(response);
...
var PaginatedChoiceState = ChoiceState.extend(function(self, name, opts) {
    ...
    :param int opts.options_per_page:
        maximum number of choices to display per page. Default is ``8``. If this
        option is explicitly given as ``null``, :class:`PaginatedChoiceState`
        will automatically divide up the given choices to fit within the
        character limit given by the ``'characters_per_page'`` option.
    :param int opts.characters_per_page:
        maximum number of characters to display per page. Default is `null` (i.e. no
        maximum), or ``160`` if the ``'characters_per_page'`` option is
        explicitly given as ``null``.
    :param object opts.events:
        Optional event name-listener mappings to bind.
    */
    opts = _.defaults(opts || {}, {
    ...
    _.defaults(opts, {
        characters_per_page: opts.options_per_page !== null
            ? null
            : 160
    });
    ...
    self.init = function() {
        ...
        if (!('page_start' in self.metadata)) {
    ...
    self.current_choices = function() {
        ...
        if (end < self.choices.length)
            choices.push(self.more);

        if (start > 0)
            choices.push(self.back);
    ...
    self._page_size = function() {
        ...
        while (++n < self.choices.length) {
            ...
            if (chars < 0) {
                // check if next choice is the last choice
                if (n+1 === self.choices.length) {
                    // check if last choice fits in 'More' space
                    chars += self.format_choice(self.more, self.choices.length).length;
                    if (chars >= 0) {
                        n++;  // incr n if it fits
                    }
                }
    ...
    self._prev_page_size = function() {
        ...
        while (n-- > 0) {
            ...
            if (chars < 0) {
                // check if previous choice is the first choice
                if (n-1 === -1) {
                    // check if first choice fits in 'Back' space
                    chars += self.format_choice(self.back, self.choices.length).length;
                    if (chars >= 0) {
                        n--;  // decr n if it fits
                    }
                }
    ...
    self.process_choice = function(choice) {
        ...
        if (choice === self.more) diff = self._page_size();
        else if (choice === self.back) diff = -self._prev_page_size();
        else return false;

        self.metadata.page_start = Math.max(0, self.metadata.page_start + diff);
        return true;
    };

    self.shorten_choices = function(text, choices) {
        if (!self.characters_per_page || !self.options_per_page)
            return choices;
        ...
        return choices.map(function (choice, index) {
            ...
            var label = choice.label.slice(0, trunc - 3) + "...";
            return new Choice(choice.value, label);
        });
    };
});
