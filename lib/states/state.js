...
var StateInvalidError = StateError.extend(function(self, state, response, opts) {
    ...
    Occurs when the state becomes invalid, either because a validation check
    failed, or becuse it was explicitly invalidated.

    :param State state: the state that caused the error.
    :param string response: the response to send back to the user.
    ...
    */
    ...
    self.translate = function(i18n) {
        self.response = i18n.gettext(self.response);
    };
...
var State = Eventable.extend(function(self, name, opts) {
    ...
    :param function opts.check:
        ...
        ``content`` is the user's input. If a string is returned, the string
        will be taken as the error response to send back to the user. If a
        :class:`StateInvalidError` is returned, its ``response`` property
        will be taken as the error response to send back to the user. Any
        other value returned will be taken as a non-error. The result may
