...
function FreeText(name, next, question, opts) {
     ...
     *     - ``next(content)`` (``Function`` or ``String``): The state that the user
     *     should visit after this state. Functions may return a promise.
     *         - ``content`` (``String``): The text from the user
     *     - ``question`` (``String``): The text to display to the user
     *     - ``opts`` (``Object``):
     *         - ``check(content)`` (``Function``): Allows one to validate a
     *         user's response. Should return ``true`` if the response is
     *         considered valid, and ``false`` if otherwise
     *             - ``content`` (``String``): The text from the user
     *         - ``error`` (``String``): The error text to display to the user
     *         if we reach this state in error
     *         - [``State`` options]
     **/
