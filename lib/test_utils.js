...
function make_im(opts) {
    opts = utils.set_defaults(opts || {}, {
        api: new DummyApi(),
        state_creator: new StateCreator('start'),
        kv: require('../test/fixtures/simple-kv'),
        config: require('../test/fixtures/simple-config'),
        msg: require('../test/fixtures/simple-msg')
    });

    var im = new InteractionMachine(opts.api, opts.state_creator);
    im.api.config_store = opts.config;
    im.api.kv_store = opts.kv;
    return im.setup(opts.msg);
}

this.ImTester = ImTester;
this.DummyIm = DummyIm;
this.DummyI8n = DummyI8n;
