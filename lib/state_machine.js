...
function StateCreator(start_state) {
    ...
    self.error_state_creator = function() {
    ...
    self.switch_to_start = function(im) {
        var creator = self.state_creators[self.start_state];

        if (creator) {
            return maybe_promise(creator.call(self, self.start_state, im));
        }

        im.log("Unknown start state '" + self.start_state + "'. " +
               "Switching to" + " error state.");
        return maybe_promise(self.error_state_creator.call(self));
    };

    self.switch_state = function(state_name, im) {
        if (typeof state_name == "undefined") {
            // handles new users who have no current state
            return self.switch_to_start(im);
        } 
