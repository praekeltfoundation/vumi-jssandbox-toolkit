...
function InteractionMachine(api, state_creator) {
    ...
    */
    ...
    self.attach = function() {
        ...
        */
        ...
        self.api.on_unknown_command = function(cmd) {
            self.emit(new UnknownCommandEvent(self, cmd)).then(done);
        };

        self.api.on_inbound_event = function(cmd) {
            self.emit(new InboundEventEvent(self, cmd)).then(done);
        };

        self.api.on_inbound_message = function(cmd) {
            self.emit(new InboundMessageEvent(self, cmd)).then(done);
        };
    };
    ...
    self.message_handlers.close = function(msg) {
        /**:InteractionMachine.message_handlers.close(msg)
    ...
    self.message_handlers.new = function(msg) {
        /**:InteractionMachine.message_handlers.new(msg)
    ...
    self.message_handlers.resume = function(msg) {
        /**:InteractionMachine.message_handlers.resume(msg)
        ...
        waits for its listeners to complete their work
            * Emits a :class:`SessionResumeEvent` on the current state and
        waits for its listeners to complete their work
            * Send a reply from the current state.
        */
        return self
    ...
    self.on('unknown_command', function(event) {
        /**:InteractionMachine.on('unknown_command', func(event))
    ...
    self.on('inbound_message', function(event) {
        /**:InteractionMachine.on('inbound_message', func(event))
