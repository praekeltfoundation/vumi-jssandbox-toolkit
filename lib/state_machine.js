...
function SessionResumeEvent(im) {
    ...
    IMEvent fired when a new user message arrives for an existing
    user session.
...
function NewUserEvent(im, user_addr, user) {
    ...
    IMEvent fired when a message arrives from a user for whom
...
function InboundEventEvent(im, event) {
    ...
    IMEvent fired when an message status event is received. Typically
    this is either an acknowledgement or a delivery report for an
    outbound message that was sent from the sandbox application.
...
function InteractionMachine(api, state_creator) {
    ...
    serves as a bridge between a `StateCreator` (i.e. set of states defining
    an application) and resources provided by the sandbox API.
    */
...
function StateCreator(start_state) {
    ...
    */
    ...
    self.on_event = function(event) {
        ...
        Called by the interaction machine when an IMEvent
    ...
    self.start_state_creator = function(state_name, im) {
        ...
        ``state_name`` and calls that. If no such creatorexists, it calls
    ...
    self.switch_state = function(state_name, im) {
        ...
        It may be overridden by StateCreator subclasses that wish to
        provide a completely dynamic set of states.
        */
        var creator;
