...
function ConfigReadEvent(im, config) {
    ...
    :js:class:`IMEvent` fired immediately after sandbox configuration is read.
...
function SessionNewEvent(im) {
    ...
    :js:class:`IMEvent` fired when a new user session starts.
...
function SessionCloseEvent(im, possible_timeout) {
    ...
    :js:class:`IMEvent` fired when a user session ends.
...
function SessionResumeEvent(im) {
    ...
    :js:class:`IMEvent` fired when a new user message arrives for an existing
    user session.
...
function NewUserEvent(im, user_addr, user) {
    ...
    :js:class:`IMEvent` fired when a message arrives from a user for whom
...
function StateEnterEvent(im, state) {
    ...
    :js:class:`IMEvent` fired when a user enters a state from a different
    state.
...
function StateExitEvent(im, state, user) {
    ...
    :js:class:`IMEvent` fired when a user exits a state to a different state.
...
function InboundEventEvent(im, event) {
    ...
    :js:class:`IMEvent` fired when an message status event is
...
function InteractionMachine(api, state_creator) {
    ...
    serves as a bridge between a :js:class:`StateCreator` (i.e. set of states
    ...
    */
    ...
    self.set_user_answer = function(state_name, answer) {
        ...
        This is called by :js:class:`State` objects when they determine that
...
function StateCreator(start_state) {
    ...
    */
    ...
    self.on_event = function(event) {
        ...
        Called by the interaction machine when an :js:class:`IMEvent`
    ...
    self.switch_state = function(state_name, im) {
        ...
        It may be overridden by :js:class:`StateCreator` subclasses that wish
