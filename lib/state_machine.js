...
function ConfigReadEvent(im) {
    return new IMEvent('config_read', im, {});
}
...
function InboundEventEvent(im) {
    return new IMEvent('inbound_event', im, {});
}
...
function InteractionMachine(api, state_creator) {
    ...
    self.on_inbound_event = function(cmd) {
        ...
        var p = new Promise();
        p.add_callback(function() {
            return self.on_event(new InboundEventEvent(self));
        });
        p.add_callback(self.done);
        return p;
    };
...
function StateCreator(start_state) {
    ...
    self.switch_state = function(state_name, im) {
        ...
        if (!self.state_creators[state_name]) {
            throw new StateError("Unknown state '" + state_name + "'");
        }
