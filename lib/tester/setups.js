...
var SetupTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.finalise.config = function(api, data) {
        _.defaults(data.config, fixtures.config());
        data.sandbox_config.config = JSON.stringify(data.config);
        _.defaults(api.config_store, data.sandbox_config);
    };

    self.finalise.user = function(api, data) {
        var defaults = _.defaults(fixtures.user(), {
            store_name: data.config.user_store_name || data.config.name
        });
    ...
    self.methods.setup.user.answers = function(answers) {
        ...
        */
        self.data.user.answers = _.defaults(
    ...
    self.methods.setup.config = function(v) {
        ...
        */
        ...
        :param function fn:
            ...
            current config data and ``this`` is the :class:`AppTester`
            instance. The config data is then set with ``fn``'s result. May
            return its result via a promise.
    ...
    self.methods.setup.kv = function(v) {
        ...
        */
        ...
        Passes the current api's kv store to the function ``fn`` to allow
        for custom setup.

        :param function fn:
            function of the form ``func(kv)``, where ``kv`` is the api's
            current kv current :class:`DummyKvResource` and ``this`` is the
            :class:`AppTester` instance. May return a promise.
