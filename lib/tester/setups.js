...
var SetupTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.finalise.config = function(api, data) {
        ...
        data.config = utils.set_defaults(
            api.config_store.config,
            data.config);

        data.sandbox_config = utils.set_defaults(
            api.config_store,
            data.sandbox_config);
    };
    ...
    self.methods.setup.user = function(v) {
        ...
        */
        ...
        updates it with the function's result.
    ...
    self.methods.setup.config = function(v) {
        ...
        */
        ...
        Passes the current app config data to the function ``fn``, then updates
        it with the function's result.

        :param function fn:
            function of the form ``func(config)``, where ``config`` is the
            current config data. The config data is then updated with ``fn``'s
        ...
        */
        return Q(utils.functor(v))
           ...
           .then(function(result) {
               utils.update(self.data.config, result || {});
           });
    };
    ...
    self.methods.setup.kv = function(v) {
        ...
        */
        ...
        Passes the current kv store data to the function ``fn``, then updates
        it with the function's result.

        :param function fn:
            function of the form ``func(kv)``, where ``kv`` is the current kv
            store data data. The kv store is then updated with ``fn``'s result.
        ...
        */
        return Q(utils.functor(v))
           .fcall(self.data.kv)
           .then(function(result) {
               utils.update(self.data.kv, result || {});
           });
    };
});
