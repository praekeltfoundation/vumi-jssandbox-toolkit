...
var SetupTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.methods.setup.user.state = function(name, opts) {
        ...
        */
        var state = new StateData(name, opts);
    ...
    self.methods.setup.config = function(v, opts) {
        ...
        */
        ...
        _.forEach(v, function(value, key) {
            self.api.config.json[key] = opts.json;
        });
    };
    ...
    self.methods.setup.config.app = function(v) {
        ...
        Updates the sandbox's app config (the ``'app'`` field in the sandbox
    ...
    self.methods.setup.config.endpoint = function(endpoint, opts) {
        /**function:AppTester.setup.endpoint(endpoint, delivery_class)

        Updates the sandbox's app config (the ``'config'`` field in the sandbox
        config) with the given outbound endpoints.

        :param endpoint str:
            the name of the endpoint to configure
        :param opts.delivery_class str:
            the name of the delivery class. See :meth:`ContactStore.get`
        ...
        */
        var app = _.defaults(self.api.config.app, {endpoints: {}});
        opts = opts || {};
        app.endpoints[endpoint] = opts;
    };
