...
var SetupTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.finalise.config = function(api, data) {
        utils.set_defaults(data.config, fixtures.config());
    ...
    self.methods.setup.user.state.metadata = function(metadata) {
        ...
        Any properties in the current metadata with the same names as
        properties in the new metadata will overwritten.

        :param object metadata:
            The new metadata to update the current state metadata with.
