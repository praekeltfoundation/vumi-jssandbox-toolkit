...
var CheckTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.methods.check = function(fn) {
        ...
        :param function fn:
            ...
            api instance (by default an instance of :class:`DummyApi`),
            ``app`` is the sandbox app being tested and ``this`` is the
    ...
    self.methods.check.user.properties = function(obj) {
        ...
        */
        ...
        if ('metadata' in obj) {
            self.methods.check.user.metadata(utils.pop_prop(obj, 'metadata'));
        }
    ...
    self.methods.check.state = function(v, metadata) {
        ...
        */
        ...
        Checks that once serialized, the state deep equals ``obj``.
        ...
        */
        ...
        if (typeof v == 'function') {
            ...
            self.assert.strictEqual(state.name, v, {
                msg: "Unexpected state name"
            });

            if (metadata) {
                self.methods.check.state.metadata(metadata);
            }

            return;
        }
        else if (typeof v == 'object') {
            self.assert.deepEqual(state.serialize(), v, {
