...
var CheckTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.assertion = function(fn, actual, expected, opts) {
        try {
            fn(actual, expected);
        }
        catch(e) {
            if (e instanceof assert.AssertionError) {
                self.assertion.format(e, opts);
            } 
    ...
    self.assertion.format = function(e, opts) {
        ...
        // adds this (since so many people use chaijs). We want the message to
    ...
    self.assert = function(v, opts) {
        ...
        self.assertion(assert.equal, !!v, true, opts);
    };

    self.assert.deepEqual = function(actual, expected, opts) {
        opts = utils.set_defaults(opts || {}, {diff: true});
        self.assertion(assert.deepEqual, actual, expected, opts);
    };

    self.assert.strictEqual = function(actual, expected, opts) {
        opts = utils.set_defaults(opts || {}, {diff: true});
        self.assertion(assert.strictEqual, actual, expected, opts);
    };
    ...
    self.methods.check.user = function(v) {
        ...
        :func:`AppTester.check.user.attrs`.
        ...
        */
        ...
        if (typeof v == 'object') {
            result = self.methods.check.user.attrs(v);
        }
        else if (typeof v == 'object') {
            result = fn.call(self.tester, self.im.user);
        }
    };

    self.methods.check.user.attrs = function(obj) {
        /**function:AppTester.check.user.attrs(obj)

        Checks that the attributes of the user after a sandbox run are equal to
        the expected properties given in ``obj``. Alias to
        :func:`AppTester.check.user.attrs`.
        ...
        */
        ...
        var user = self.im.user.serialize();
        utils.each_prop(obj, function(value, name) {
            ...
            if (typeof value == 'object') {
                assert.deepEqual(user[name], value);
            }
            else {
