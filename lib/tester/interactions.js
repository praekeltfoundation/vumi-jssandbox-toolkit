...
var InteractionTasks = AppTesterTasks.extend(function(self, tester) {
    ...
    self.send = function(msg, defaults) {
        _.defaults(msg, defaults || {}, fixtures.msg());
    ...
    self.send.many = function(msgs, defaults) {
        return self
            .send(msgs.shift(), defaults)
            .then(function() {
                if (!msgs.length) { return; }

                var api = self.api;
                self.tester.reset.interaction();
                _.extend(self.api.kv.store, api.kv.store);
                _.extend(self.api.config.store, api.config.store);
    ...
    self.methods.input = function(v) {
        ...
        */
        ...
        /**function:AppTester.input(input1, input2[, ...]])

        Uses each input as a new interaction. :class:`AppTester` setup methods
        will count for the first interaction, subsequent interactions will
        rely on api state from the previous interaction, and check methods
        will only happen after the last interaction.
        ...
        .. code-block:: javascript
            ...
            tester.input(null, 'coffee', '1', {content: '2'});
        */
