...
var InteractionMachine = Eventable.extend(function(self, api, app) {
    ...
    */
    ...
    self.attach = function() {
        ...
        Attaches the im the given api and app. The sandbox API's event handlers
        are set to emit the respective events on the interaction machine, then
        terminate the sandbox once their listeners are done.
        */
        var done = self.done;
        var err = self.err;
    ...
    self.switch_state = function(state_name, metadata, creator_opts) {
        ...
        The following steps are taken:
            ...
            * The IM delegates to its app's :meth:`AppStates.create`
              to create the new state.
            * Once the state has been setup, we invoke the state's
              :meth:`State.translate` with the user's current
              :class:`Translator` (which corresponds to the user's language).
        ...
        */
        ...
        return p.then(function(new_state) {
            return new_state
                .setup(self, metadata)
                .then(function() {
                    return new_state.translate(self.user.i18n);
                })
                .then(function() {
