...
var InteractionMachine = Eventable.extend(function(self, api, app) {
    ...
    */
    ...
    self.set_new_state = function(state) {
        /**:InteractionMachine.set_new_state(state)
    ...
    self.resume_state = function(state) {
        ...
        :param state:
            the state to resume
        */
        return self.set_new_state(state)
            .then(function(state) {
                return self.emit.state.resume(state);
            });
    };
    ...
    self.switch_state = function(dest) {
        ...
        The following steps are taken:
            * A :class:`StateExitEvent` is emitted for the new state (on
              :class:InteractionMachine, then the new state)
            * The destination state is created
            * The destination state is set to the current state (see
              :meth:InteractionMachine.set_state:)
            * A :class:`StateEnterEvent` is emitted for the new state (on
              :class:InteractionMachine, then the new state)
        */
        var src = self.state;
        var p = Q();
        dest = new StateData(dest);
        if (!dest.exists() || dest.is(src)) { return p; }

        return p
            .then(function() {
                if (src) return self.emit.state.exit(src);
            })
            .then(function() {
