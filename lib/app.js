...
var App = Eventable.extend(function(self, start_state_name, opts) {
    ...
    */
    ...
    opts = utils.set_defaults({AppStates: AppStates});
    ...
    */
    ...
    self.check = function(state, name) {
        ...
        */
        ...
        check(state.name == name || state.name == '__error__', [
            "Creator for state '" + name + "' created a state with",
            "a different name: '" + state.name + "'"
        ].join(' '));
    };

    self.create = function(name, opts) {
        /**:AppStates.create(name, metadata)
        ...
        :param object opts:
            Optional data for the state to be created. Optional, state creators
            can use this but don't have to.
        ...
        */
        ...
        return Q(creator.call(self.app, name, opts))
            .then(function(state) {
